"""
FTC Tokenomics Simulation v2.0
================================
Updated to match Mr. Vincent's resolved design decisions:
- Base deposit: $250
- 1:1 treasury match for qualifying low-income families (effective $500)
- Lightweight KYC at account creation, full KYC at $5,000 cumulative withdrawals
- Partner-merchant discount network (10-30%)
- Delaware C-Corp + Wyoming DAO LLC + Swiss foundation legal structure
- Target raise: $8-15M public sale, $60-80M initial market cap
- Base L2
- 21B fixed supply, no inflation, 25% of merchant fees burned
"""
...
class Tokenomics:
    ...
    base_deposit_usd: float = 250.0
    low_income_match_rate: float = 1.0  # 1:1 match
    low_income_eligible_pct: float = 0.40  # 40% of families qualify for match
    match_cap_usd: float = 250.0  # max match = $250
    ...
    discount_range_min_pct: float = 0.10
    discount_range_max_pct: float = 0.30
    ...
    # --- Valuation ---
    initial_market_cap_usd: float = 70_000_000.0  # midpoint of $60-80M range
    ...
    def effective_discount(self, total_children: float) -> float:
        """Discount scales with adoption: 10% early, up to 30% at scale."""
        frac = min(total_children / self.max_children, 1.0)
        return self.discount_range_min_pct + (self.discount_range_max_pct - self.discount_range_min_pct) * frac
    ...
    def run(self) -> pd.DataFrame:
        ...
        for year in range(self.years + 1):
            ...
            low_income_kids = new_kids * self.low_income_eligible_pct
            matched_kids = low_income_kids
            deposits = new_kids * self.base_deposit_usd
            match = matched_kids * min(self.base_deposit_usd * self.low_income_match_rate, self.match_cap_usd)
            ...
            current_discount = self.effective_discount(total_children)
            ...
            records.append({
                ...
                "avg_discount_pct": current_discount * 100,
                "price_usd": price,
    ...
    def plot(self, df: pd.DataFrame, scenario_name: str = "Base Case") -> None:
        ...
        fig.suptitle(f"FTC Tokenomics v2.0 — {scenario_name}", fontsize=14, fontweight="bold")
        ...
        out_path = f"C:/Users/bidbu/ftc/models/tokenomics_v2_{safe}.png"
        plt.tight_layout()
...
def main():
    scenarios = {
        "Conservative": Tokenomics(
            ...
            initial_market_cap_usd=60_000_000,
        ),
        "Base Case": Tokenomics(
            ...
            initial_market_cap_usd=70_000_000,
        ),
        "Abundance": Tokenomics(
            ...
            initial_market_cap_usd=80_000_000,
            discount_range_max_pct=0.45,
        ),
        "Adverse": Tokenomics(
    ...
    print("\n=== 30-Year Scenario Summary (v2.0) ===")
    print(summary.to_string(index=False))
    summary.to_csv("C:/Users/bidbu/ftc/models/scenario_summary_v2.csv", index=False)
